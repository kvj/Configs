...
function! s:Project(filename) " <<<
    ...
    if !exists("g:proj_running") || (bufwinnr(g:proj_running) == -1) " Open the Project Window
        ...
        setlocal nomodeline
        if exists("g:proj_after_open")
            exec g:proj_after_open
        endif
    else
        silent! 99wincmd h
    ...
    function! s:OpenEntry(line, precmd, editcmd, dir)
        "echo 'OpenEntry ['.a:line.'] ['.a:precmd.'] ['.a:editcmd.'] ['.a:dir.']'
    ...
    function! s:OpenEntry2(line, infoline, fname, editcmd)
        ...
        "echo 'OpenEntry2 '.a:line.' '.cd_cmd.' '.fname.' '.a:editcmd
        if a:editcmd[0] != '' " If editcmd is '', then just set up the environment in the Project Window
            ...
            let cmdsuff = ''
            if g:proj_last_buffer
                "let cmdsuff = ' #'.g:proj_last_buffer
            endif
            if s:IsAbsolutePath(fname) == 2
                exec a:editcmd.' '.fname.cmdsuff
            else
                silent exec 'silent '.a:editcmd.' '.fname.cmdsuff
            endif
    ...
    function! s:DoEntryFromDir(recursive, line, name, absolute_dir, dir, c_d, filter_directive, filter, foldlev, sort)
        let res = s:GenerateEntry(a:recursive, a:line, a:name, escape(a:absolute_dir, ' '), escape(a:dir, ' '), escape(a:c_d, ' '), a:filter_directive, a:filter, a:foldlev, a:sort)
        normal! j
    ...
    function! s:CreateEntriesFromDir(recursive, createdirmode)
        " Save a mark for the current cursor position
        normal! mk
        let line=line('.')
        let name = ''
        if a:createdirmode != 1
        ...
        if a:createdirmode 
            let name = dir
            let name = matchstr(name, '\(.*\)[/|\\\\|$]')
            let name = substitute(name, '[/|\\\\]$', '', '')
            if strlen(name) == 0
                call confirm('"'.dir.'" is not valid directory.', "&OK", 1)
                return
            endif
            let dir = name
        else
    ...
    function! s:RefreshEntriesFromDir(recursive, fullrefresh)
        ...
        "echo 'Refresh '.a:recursive.' '.a:fullrefresh.' '.line('.').' '.home
        if home == ''
        ...
        while getline('.') !~ '}'
            ...
            if getline('.') !~ '{'
                ...
                if a:fullrefresh == 1
                    ...
                    if a:recursive == 1
                        call s:RefreshEntriesFromDir(1, 0)
                        "echo 'After refresh cursor '.line('.').' '.getline('.').' '.foldlev.' '.foldlevel('.')
                        if foldlevel('.') > foldlev
                            normal! ]zj
                        else
                            "echo 'Empty folder '.home
                            normal! j
                        endif
                        "echo 'After refresh cursor '.line('.').' '.getline('.')
                    else
                        "close sub-fold
                        if foldclosed('.') == -1
                            normal! zc
            ...
            "echo 'afterloop '.getline('.').' '.line('.').' '.home
        endwhile
        if just_a_fold == 0
            ...
            if !isdirectory(glob(home))
                ...
                if a:fullrefresh == 1
                    let line = line('.')-1
                    call s:VimDirListing('*', '', "\010", 's:f2', 's:filecount', 's:dirs', 's:dcount')
                    "echo 'Doing refresh folders '.s:dcount.' '.s:filecount
                    if s:dcount > 0
                        cd -
                        while s:dcount > 0
                            let dname = substitute(s:dirs,  '\(\( \|\f\|:\)*\).*', '\1', '')
                            let edname = escape(dname, ' ')
                            let s:dirs = substitute(s:dirs, '\( \|\f\|:\)*.\(.*\)', '\2', '')
                            "echo 'Gen '.line.' '.dname.' '.home.' '.edname.' '.filter.' '.foldlev.' '.sort
                            let line = s:GenerateEntry(1, line+1, dname, home.'/'.edname, edname, '', '', filter, foldlev, sort)
                            let s:dcount=s:dcount-1
                        endwhile
                    endif
                    exec 'cd '.cwd
                    "normal! `l
                    "echo 'Refreshing all folders '.line('.').' '.home.' '.cwd
                    call s:RefreshEntriesFromDir(1, 0)
                else
                    call s:VimDirListing(filter, spaces, "\n", 'b:files', 'b:filecount', 'b:dirs', 'b:dircount')
                    "echo 'Refreshing files '.line('.').' '.home.' '.b:filecount
                    if b:filecount > 0
                        normal! mk
                        silent! put =b:files
                        normal! `kj
                    ...
                    unlet b:files b:filecount b:dirs b:dircount
                    "echo 'Just print and sort'
                    exec 'cd '.cwd
                    "normal! `l
                    "echo 'Refresh done '.line('.').' '.home
                endif
            endif
        endif
        " Go to the top of the refreshed fold.
        "normal! `lj
        "echo 'Finally '.line('.').' '.home
    endfunction ">>>
    " s:MoveUp() <<<
    ...
    if !exists("g:proj_running")
        ...
        nnoremap <buffer>          <LocalLeader>F0 \|:call <SID>ListSpawn("_fold")<CR>
        nnoremap <buffer> <silent> <LocalLeader>c :call <SID>CreateEntriesFromDir(0, 0)<CR>
        nnoremap <buffer> <silent> <LocalLeader>C :call <SID>CreateEntriesFromDir(1, 0)<CR>
        nnoremap <buffer> <silent> <LocalLeader>D :call <SID>CreateEntriesFromDir(1, 1)<CR>
        ...
        nnoremap <buffer> <silent> <LocalLeader>L :call <SID>RefreshEntriesFromDir(1, 1)<CR>
...
" vim800: set foldmethod=marker foldmarker=<<<,>>> foldlevel=1:
